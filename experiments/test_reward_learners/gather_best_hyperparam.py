...
def process_metrics(
    run_id: str, num_elements_per_list: int = 100
) -> dict[str, Union[float, np.ndarray]]:
    """Extract the metrics of a single run.

    Args:
        run_id (str): The id of the run to extract the metrics from.
        num_elements_per_list (int, optional): The minimum number of
            times each metric has to be computed in order to be included
            in the results. Defaults to 100.

    Returns:
        dict[str, Union[float, np.ndarray]]: A dictionary containing the extracted
            metrics of the specified run.
    """
    ...
    for metric_base_name in metric_base_names:
        ...
        these_metrics = [m for m in metric_names if m[: m.index("_seed")] == metric_base_name]
        for metric_name in these_metrics:
            # Extract the metric and append it to the temp list
            metric_list = client.get_metric_history(run_id, metric_name)
            max_length = max(max_length, len(metric_list))
            length_sum += len(metric_list)
            metrics_temp.append(
                [m.value for m in metric_list]
                + (num_elements_per_list - len(metric_list)) * [np.nan]
            )

        # Average the metrics
        metrics_temp_np = np.array(metrics_temp)
        metrics_temp_np = metrics_temp_np[:, :max_length]
        metrics[metric_base_name] = np.nanmean(metrics_temp_np, axis=0)
        metrics[metric_base_name + " average length"] = length_sum / len(these_metrics)
...
def process_metrics_distributed(
    ...
    Returns:
        dict[str, dict[str, dict[str, Union[float, np.ndarray]]]]: A dictionary
            containing the extracted metrics of all runs belonging to the
    ...
    for key in run_ids:
        ...
        if isinstance(run_ids[key], str):
            ...
            for run_name in run_ids[key]:
                ...
                # try:
                metrics_dic = process_metrics(run_ids[key][run_name], num_elements_per_list)
...
def find_best_runs(
    metrics: dict[str, dict[str, dict[str, Union[float, np.ndarray]]]],
    metric_name: str,
    ...
    for key in metrics:
        ...
        result_lists[key] = sorted(result_list, reverse=True)
...
if __name__ == "__main__":
    ...
    main_run_id = "aa6ce302d11540dea34dd230c3c1ad29"
    ...
    num_elements_per_list = 100
    min_average_elements_per_list = 1
    ...
    metrics = process_metrics_distributed(
        ...
        num_elements_per_list=num_elements_per_list,
    )
    ...
    for key in metrics:
        ...
        for run_name in metrics[key]:
            ...
            if "log likelihood" not in set(metrics[key][run_name].keys()):
                to_delete.append(run_name)

            # Remove runs which repeatedly crashed
            elif (
                "log likelihood average length" not in set(metrics[key][run_name].keys())
                or metrics[key][run_name]["log likelihood average length"]
                < min_average_elements_per_list
            ):
                to_delete.append(run_name)
    ...
    best_runs, all_runs = find_best_runs(
        metrics, metric_name="log likelihood", direction="largest", mode="last"
    )
