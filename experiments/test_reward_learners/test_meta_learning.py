...
def test_reward_learners(
    ...
    suffix: str = "",
) -> tuple[EnvironmentBase, RewardModelPacohGP, float, float, float, float]:
    """The main function of the experiment. Meta-trains and tests a reward learner.

    Args:
        train_size (int): The training dataset size (Unused)
        test_size (int): The test dataset size (Unused)
        ...
        meta_train (bool, optional): Whether to meta-train the reward learner. (Must
            be always True). Defaults to False.
    ...
    Returns:
        tuple[
            ...
            float,
            float
        ]: The environment, the reward learner, the log likelihood, the residual mean
            squared error, the calibration error, and the calibration error chi squared.
    """
    if seed is not None:
        set_all_seeds(seed)
    else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        args: Optional[list[str]] = (
            "--configs "
            # "3d/config_point_env_goal_fpacoh_gp.py "
