...
from meta_arl.meta_bo.models.util import find_root_by_bounding
...
class UnnormalizedExpDist(Distribution):
    ...
    def __init__(self, exponent_fn: Callable):
...
class FactorizedNormal(Distribution):
    def __init__(self, loc: np.ndarray, scale: np.ndarray, summation_axis: int = -1):
        self.normal_dist = torch.distributions.Normal(loc, scale)
        self.summation_axis = summation_axis

    def log_prob(self, value: np.ndarray) -> torch.Tensor:
...
class EqualWeightedMixtureDist(Distribution):
    def __init__(
        self,
        dists: Union[list[torch.distributions.Distribution], torch.distributions.Distribution],
        batched: bool = False,
    ...
    def mean(self) -> torch.Tensor:
        if self.batched:
            assert isinstance(self.dists, torch.distributions.Distribution)
            return torch.mean(self.dists.mean, dim=0)
        else:
            assert isinstance(self.dists, list)
    ...
    def log_prob(self, value: torch.Tensor) -> torch.Tensor:
        if self.batched:
            assert isinstance(self.dists, torch.distributions.Distribution)
            # self.dists = cast(torch.distributions.Distribution, self.dists)
...
class CatDist(Distribution):
    def __init__(
        self, dists: list[torch.distributions.Distribution], reduce_event_dim: bool = True
    ):
    ...
    def _sample(
        self, sample_shape: Union[tuple[int], torch.Size], sample_fn: str = "sample"
    ) -> torch.Tensor:
...
class NeuralNetwork(torch.nn.Sequential):
    ...
    def __init__(
        ...
        nonlinearlity: Callable = torch.tanh,
        ...
        if weight_norm:
            _normalize: Callable = torch.nn.utils.weight_norm
        else:
...
class VectorizedModel:
    ...
    def parameter_shapes(self) -> OrderedDict:
    ...
    def set_parameters(self, param_dict: dict[str, Any]) -> None:
        for name, value in param_dict.items():
            self.set_parameter(name, value)

    def parameters_as_vector(self) -> torch.Tensor:
...
class NeuralNetworkVectorized(VectorizedModel):
    ...
    def __init__(
        ...
        input_dim: int,
        output_dim: int,
        layer_sizes: tuple[int, int] = (64, 64),
        nonlinearlity: Callable = torch.tanh,
    ):
        super().__init__(input_dim, output_dim)
...
def _kaiming_uniform_batched(
    tensor: torch.Tensor, fan: int, a: float = 0.0, nonlinearity: str = "tanh"
) -> torch.Tensor:
...
class ConstantMeanLight(gpytorch.means.Mean):
    def __init__(
        self, constant: torch.Tensor = torch.ones(1), batch_shape: torch.Size = torch.Size()
    ):
...
class SEKernelLight(gpytorch.kernels.Kernel):
    def __init__(
        self,
        lengthscale: torch.Tensor = torch.tensor([1.0]),
        output_scale: torch.Tensor = torch.tensor(1.0),
    ):
...
class HomoskedasticNoiseLight(gpytorch.likelihoods.noise_models._HomoskedasticNoiseBase):
    def __init__(self, noise_var: torch.Tensor, *params: Any, **kwargs: Any):
        self.noise_var = noise_var
        self._modules: Dict[Any, Any] = {}
...
class GaussianLikelihoodLight(gpytorch.likelihoods._GaussianLikelihoodBase):
    def __init__(self, noise_var: torch.Tensor, batch_shape: torch.Size = torch.Size()):
        self.batch_shape = batch_shape
        self._modules: Dict[Any, Any] = {}
        self._parameters: Dict[Any, Any] = {}
...
class LearnedGPRegressionModel(gpytorch.models.ExactGP):
    ...
    def __init__(
        ...
        learned_kernel: Optional[NeuralNetwork] = None,
        learned_mean: Optional[NeuralNetwork] = None,
        mean_module: Optional[gpytorch.means.Mean] = None,
        covar_module: Optional[Callable] = None,
    ):
        ...
        # DEBUG START
        self.not_positive_definite_counter = 0
        # DEBUG END
    ...
    def forward(self, x: torch.Tensor) -> gpytorch.distributions.MultivariateNormal:
        ...
        assert self.covar_module is not None
        ...
        # First extract whole matrix
        covar_x = covar_x.evaluate()

        # Force it to be symmetric
        covar_x = (covar_x + covar_x.T) / 2
        ...
        inject_noise_std = 1e-3
        base_covariance = covar_x
        covar_x_fixed = base_covariance  # we start with 0 jitter for this matrix
        for error_counter_posterior in range(9):
            if is_positive_definite(covar_x_fixed):
                break

            inject_noise_std = 2 * inject_noise_std

            covar_x_fixed = base_covariance + inject_noise_std * torch.eye(
                base_covariance.shape[0]
            )

            warnings.warn(
                "Provided covariance matrix was not positive definite! "
                "Doubling inject_noise_posterior_std to %.4f and retrying" % (inject_noise_std)
            )
            self.not_positive_definite_counter += 1

        # This else belongs to the for- statement not the if- statement!
        else:
            raise RuntimeError("Not able to create positive definite covariance matrix!")

        # Convert it back to LazyTensor format
        covar_x_fixed = gpytorch.lazy.NonLazyTensor(covar_x_fixed)

        #####################################
        #            End of fix             #
        #####################################

        return gpytorch.distributions.MultivariateNormal(mean_x, covar_x_fixed)
    ...
    def pred_dist(self, x: torch.Tensor) -> torch.distributions.Distribution:
        self.eval()
        return self.likelihood(self.__call__(x))

    def pred_ll(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
...
class LearnedGPRegressionModelApproximate(ApproximateGP):
    ...
    def pred_dist(self, x: torch.Tensor) -> torch.Tensor:
        self.eval()
        return self.likelihood(self.__call__(x))
    ...
    @property
    def variational_distribution(self) -> gpytorch.distributions.Distribution:
